...
export default function Home() {
  ...
  useEffect(() => {
    ...
    game.on("stateChange", (state: string) => {
      ...
      if (state === "gameover") {
        console.log(
          "Calling saveToLeaderboard, score:",
          scoreRef.current,
          "level:",
          levelRef.current,
        );
  ...
  const saveToLeaderboard = async (
    ...
    console.log("saveToLeaderboard called:", {
      finalScore,
      levelsCompleted,
      mapName,
      user: userRef.current?.email,
  ...
  const startGame = async (
    ...
    if (gameRef.current) {
      ...
      const mapSize = customMap
        ? Math.max(customMap.width, customMap.height)
        : DEFAULT_MAP_SIZE;
        ...
        <div>
          {user ? (
            <span className="text-neon-green">
              {t("welcome")},{" "}
              <span className="text-neon-cyan">{user.email}</span>
            </span>
          ) : (
      ...
      <div className="relative px-4">
        ...
        {(gameState === "playing" ||
          ...
          <div className="absolute -top-12 left-0 right-0 flex justify-between items-center text-white px-4 z-10 bg-black/80 border-b-2 border-neon-cyan/50 py-2">
            <div className="flex items-center gap-6">
              <span className="text-neon-yellow font-bold text-lg drop-shadow-[0_0_5px_#ffff00]">
                SCORE: {score}
              </span>
              <span className="text-neon-cyan font-bold text-lg drop-shadow-[0_0_5px_#00ffff]">
                LEVEL: {level}
              </span>
            </div>
            <div className="flex items-center gap-4">
              {gameMode === GameMode.MULTIPLAYER ? (
                <div className="flex items-center gap-4">
                  <span className="text-neon-green font-bold drop-shadow-[0_0_5px_#00ff00]">
                    P1:{" "}
                    {playerLives.player1 === 0
                      ? "☠️"
                      : "❤️".repeat(playerLives.player1)}
                  </span>
                  <span className="text-neon-yellow font-bold drop-shadow-[0_0_5px_#ffff00]">
                    P2:{" "}
                    {playerLives.player2 === 0
                      ? "☠️"
                      : "❤️".repeat(playerLives.player2)}
                  </span>
                </div>
              ) : (
                <span className="text-neon-red font-bold drop-shadow-[0_0_5px_#ff0000]">
                  LIVES: {lives === 0 ? "☠️" : "❤️".repeat(lives)}
                </span>
              )}
            </div>
          </div>
        ...
        {gameState === "menu" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/90">
            <h1
              className="text-5xl font-bold text-neon-yellow mb-8 drop-shadow-[0_0_10px_#ffff00,0_0_20px_#ff0000]"
              style={{
                textShadow:
                  "0 0 10px #ffff00, 0 0 20px #ff0000, 0 0 30px #ff0000",
              }}
            >
              {t("tankBattle")}
            </h1>
            ...
            <p className="text-neon-magenta/80 mb-6 text-sm">
              {t("multiplayerControls") ||
                "双人模式: 玩家1(WASD+空格) 玩家2(方向键+0)"}
            </p>
            ...
            <div className=" gap-6 items-start">
              <div className="flex flex-col gap-3 w-52">
                ...
                <button
                  onClick={() =>
                    startGame(selectedMap || undefined, GameMode.SINGLE)
                  }
                ...
                <button
                  onClick={() =>
                    startGame(selectedMap || undefined, GameMode.MULTIPLAYER)
                  }
                ...
                <button
                  onClick={() =>
                    startGame(selectedMap || undefined, GameMode.ENDLESS)
                  }
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    ...
                    className="flex-1 flex items-center justify-center gap-1 px-2 py-1.5 bg-neon-purple/20 hover:bg-neon-purple/40 border border-neon-purple text-neon-purple font-bold rounded text-xs transition-all duration-300 hover:shadow-neon-purple whitespace-nowrap"
                  >
                    <span>🗺️</span>
        ...
        {gameState === "paused" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/90">
            <h1 className="text-4xl font-bold text-neon-cyan mb-8 drop-shadow-[0_0_10px_#00ffff]">
              {t("paused")}
            </h1>
        ...
        {gameState === "gameover" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/90">
            <h1 className="text-4xl font-bold text-neon-red mb-4 drop-shadow-[0_0_10px_#ff0000]">
              {t("gameOver")}
            </h1>
            ...
            <p className="text-neon-cyan/70 mb-8">
        ...
        {gameState === "victory" && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/90">
            <h1 className="text-4xl font-bold text-neon-yellow mb-4 drop-shadow-[0_0_10px_#ffff00,0_0_20px_#ffff00]">
              {t("victory")}
            </h1>
            <p className="text-white mb-2">
              {t("finalScore")}:{" "}
